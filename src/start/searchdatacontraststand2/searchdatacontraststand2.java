...
@CrossOrigin
@Controller
public class searchdatacontraststand2 {
    @RequestMapping(value = "searchdatacontraststand2")
    public @ResponseBody searchdatacontraststand2result searchdatacontraststand2(@RequestBody searchdatacontraststand2post sp) throws ClassNotFoundException, SQLException {
        jdbc j = new jdbc();
        ...
        searchdatacontraststand2result result = new searchdatacontraststand2result();
        searchdatacontraststand2data data = new searchdatacontraststand2data();
...
//        try {
            ...
            ps = conn.prepareStatement("SELECT * FROM channeldata WHERE dwgno = ? AND tongdaoshu = 2");
            ps.setString(1,sp.getDwgno());
            rs = ps.executeQuery();
            if(rs.next()){
            ...
            ps2 = conn.prepareStatement("SELECT * FROM channeldata WHERE dwgno = ? AND tongdaoshu = 2");
            ps2.setString(1,sp.getDwgno());
            rs2 = ps2.executeQuery();
            while (rs2.next()){
            ...
            if(aweldmaxalign.size() != 0){
                data.setAweldmaxalign(aweldmaxalign.get(0));
            }
            if(bweldmaxalign.size() != 0){
                data.setBweldmaxalign(bweldmaxalign.get(0));
            }
            if(aweldmaxangul.size() != 0){
                data.setAweldmaxangul(aweldmaxangul.get(0));
            }
            if(bweldmaxangul.size() != 0){
                data.setBweldmaxangul(bweldmaxangul.get(0));
            }
            if(weldreinfd.size() != 0){
                data.setWeldreinfd(weldreinfd.get(0));
            }
            if(weldreinfs.size() != 0){
                data.setWeldreinfs(weldreinfs.get(0));
            }
            if(innerdia.size() != 0){
                data.setInnerdia(innerdia.get(0));
            }
            if(straightness.size() != 0){
                data.setStraightness(straightness.get(0));
            }
            if(outward.size() != 0){
                data.setOutward(outward.get(0));
            }
            if(concave.size() != 0){
                data.setConcave(concave.get(0));
            }
            if(roundness.size() != 0){
