...
@CrossOrigin
@Controller
public class putchanneldata {                                           //提交通道数据信息
    @RequestMapping(value = "putchanneldata")
    public @ResponseBody putchanneldataresult putchanneldata(@RequestBody putchanneldatapost pp) throws ClassNotFoundException, SQLException {
        jdbc j = new jdbc();
        ...
        try {
            ...
            for (int i = 0;i<pp.getData().size();i++){
                ...
                ps1 = conn.prepareStatement("SELECT * FROM channeldata WHERE dwgno = ? AND name = ? AND status = 0");
                ...
                if(rs1.next()){
                    ...
                    sql = "INSERT INTO channeldata(dwgno" +sql1+
