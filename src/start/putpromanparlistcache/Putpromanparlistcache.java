package start.putpromanparlistcache;
...
@CrossOrigin
@Controller
public class Putpromanparlistcache {                                 //产品制造参数提交
    @RequestMapping(value = "putpromanparlistcache")
    public @ResponseBody
    putpromanparlistcacheresult putpromanparlist(@RequestBody putpromanparlistcachepost pp) throws ClassNotFoundException, SQLException {
        jdbc j = new jdbc();
        ...
        putpromanparlistcacheresult result = new putpromanparlistcacheresult();


        try {

            ps = conn.prepareStatement("SELECT * FROM promanparlistcache WHERE prodno = ? AND status = 1");
            ps.setString(1,pp.getProdno());
            rs = ps.executeQuery();
            if(rs.next()){
                ps = conn.prepareStatement("UPDATE promanparlistcache SET prodno=?,proparlist_id_dwgno=?," +
                        ...
                        "minthickstand=?,minthick=?,outward=?,concave=? WHERE prodno = ? AND status = 1");
                ps.setString(1,pp.getProdno());
                ...
                ps.setDate(25,new Date(new java.util.Date().getTime()));
                ...
                ps.setString(38,pp.getProdno());
                ps.executeUpdate();
                ps.close();
            }else {
                ps = conn.prepareStatement("INSERT INTO promanparlistcache(prodno,proparlist_id_dwgno," +
                        "orderunit,eorderunit,ecode,dealter," +
                        "submatl,docum,dedate,blankdate," +
                        "matlretest,specmatl,overmatl,nwpq," +
                        "nprocess,copsitu,exworkdate," +
