...
@CrossOrigin
@Controller
public class getbarometricreport {                                          //气压试验报告
    @RequestMapping(value = "getbarometricreport")
    public @ResponseBody
    ResponseEntity<byte[]> getbarometricreport(String prodno, String name, HttpServletRequest request) throws IOException, ParseException, SQLException, ClassNotFoundException {
        ...
        File realfile = new File(uploadPath,"气压试验报告.xlsx");
        InputStream inputStream = new FileInputStream(realfile.getAbsoluteFile());                           //服务器根目录的路径

        String filename = "气压试验报告_copy.xlsx";                                 //将文件上传的服务器根目录下的upload文件夹
        ...
        headers.setContentDispositionFormData("attachment", "气压试验报告.xlsx");// 文件的属性，也就是文件叫什么吧
