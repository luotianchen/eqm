...
public class putpromanparlist {                                 //产品制造参数提交
    ...
    public @ResponseBody putpromanparlistresult putpromanparlist(@RequestBody putpromanparlistpost pp) throws ClassNotFoundException, SQLException {
        ...
        try {
            ...
            ps = conn.prepareStatement("SELECT * FROM promanparlist WHERE prodno = ? AND status = 0");
            ps.setString(1,pp.getProdno());
            rs = ps.executeQuery();
            if(rs.next()){
                ps = conn.prepareStatement("UPDATE promanparlist SET prodno=?,proparlist_id_dwgno=?," +
                        ...
                        "minthickstand=?,minthick=?,outward=?,concave=? WHERE prodno = ? AND status = 0");
                ps.setString(1,pp.getProdno());
